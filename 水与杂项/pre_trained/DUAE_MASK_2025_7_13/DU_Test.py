...
def main():
    ...
    Normal_Dataset = modules.DUAE_Dataset(
    ...
    Pneumonia_Dataset = modules.DUAE_Dataset(
    ...
    criterion = nn.L1Loss()
    net.load_state_dict(torch.load('pth/model_epoch_1000.pth'))
    net.eval()
    ...
    normal_top = compute_top_percent(normal_mses,percent=0.01)
    ...
    print(f"- Mean of top 1% MSE: {normal_top:.6f}")
    ...
    plt.axvline(x=normal_top, color='blue', linestyle='--', label=f'Threshold ({normal_top})')
